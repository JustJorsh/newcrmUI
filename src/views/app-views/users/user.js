...
import { Row, Col, Card, Tabs, Table, Divider, Modal, Menu, message, Tag, Button, Popconfirm, Switch } from 'antd';
import { Icon } from 'components/util-components/Icon';
import { MailOutlined, PhoneOutlined, StarOutlined, UserOutlined, LinkOutlined, EyeOutlined, DeleteOutlined, CompassOutlined, MessageOutlined } from '@ant-design/icons';
...
const User = () => {
  ...
  const deleteProfileUpdate = async (id) => {
    ...
    try {
      const client = await AxiosClient();
      const response = await client.delete(`customers/updates/${id}/${params.id}`);
  ...
  const fetchUserDetails = async (Mobile) => {
    ...
    try {
      ...
      const client = await AxiosClient();
      const response = await client.get(`customers/list/${Mobile}`);
  ...
  const fetchSatisfactorysurvey = async (Mobile) => {
    ...
    try {
      if (Mobile.indexOf("0") === 0) Mobile = Mobile.substr(1);
      const client = await AxiosClient();
      const response = await client.get(`https://www.eskabatch.services.heirslifeassurance.com/api/v1/cx/getCustomerCSat?email=tetstststdggd@gmail.com${Mobile}`);
  ...
  const onChange = (checked) => {
    ...
    let company = 0;
    if (checked == true) company = 2;
    else {
      company = 1;
    }
    let cu = JSON.parse(localStorage.getItem("HH-user"));

    cu.CompanyId = company;

    localStorage.setItem("HH-user", JSON.stringify(cu));

    setCustomers(cu);
    fetchTickets(`${cu?.Email}`);
    fetchClaims(params.id);
    ...
    if (cu?.CompanyId == 2) {
      fetchAllStatement(params.id);
    }
    if (cu?.CompanyId == 2) {
      fetchPolicyLife(params.id);
    } else {
      fetchPolicyGeneral(params.id);
    }

    if (cu?.CompanyId == 2) {
      lifefetchAllDocument(params.id);
    } else {
      generalfetchAllDocument(params.id);
    }
  ...
  useEffect(() => {
    ...
    setCus(JSON.parse(localStorage.getItem("HH-user")));
    const cu = JSON.parse(localStorage.getItem("HH-user"));
              ...
              <Row gutter='16'>
                <Col xs={24} sm={24} md={8}>
                  <Row className='mb-2'>
                    ...
                    <Col xs={12} sm={12} md={12}>
                      <span className='font-weight-semibold'>
                        {user?.CompanyId == 2 ? 'Heirs Life Assurance' : 'Heirs General'}
                      </span>
                    </Col>
                  </Row>
                  <Row className='mb-2'>
                  ...
                  <Row className='mb-2'>
                    ...
                    <Col xs={12} sm={12} md={12}>
                      <span>{profileUpdates[0]?.record || ""}</span>
                    </Col>
                  </Row>
                </Col>
              </Row>
  ...
  const ProfileUpdates = (
    <Card hidden={active !== 'profile_updates'}>
      <Flex alignItems="center" justifyContent="between" mobileFlex={false}>
        ...
        <div>
          <Button onClick={() => setProfile({ user_id: params?.id, company_id: user?.CompanyId })} type="primary" >Add Note</Button>
        </div>
      </Flex>
      <div className='table-responsive'>
